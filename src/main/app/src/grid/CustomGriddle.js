...
export default class CustomGriddle extends Component {
    constructor(props, context) {
        super(props, context);
        this.griddleName = props.modelName;
    ...
    getInitialSearch(filters) {
        if (JSON.stringify(filters) === JSON.stringify({})) {
            this.props.dataStore.search(
                {},
                this.getOffset(getSearchPage(this.griddleName)),
                this.getSize(getSearchPage(this.griddleName))
            );
        } else {
            this.props.dataStore.search(
                filters,
    ...
    getOffset(searchPageState) {
        return searchPageState.offset
            ? searchPageState.offset
            : this.props.dataStore.offset;
    }

    getSize(searchPageState) {
        return searchPageState.size
            ? searchPageState.size
            : this.props.dataStore.size;
    }
    ...
    render() {
        ...
        return (
            <Griddle
                ...
                components={{
                    Filter: () => (
                        <div className="float-top col-xs-12 col-sm-8 col-sm-offset-4 col-md-9 col-md-offset-3 col-lg-10 col-lg-offset-2">
                            <CustomFilter
                                setFilter={filterObject =>
                                    this._onFilter(filterObject)
                                }
                                filters={this.getFilters(
                                    getSearchPage(this.griddleName)
                                )}
                            />
                        </div>
                    ),
    ...
    _onFilter = newFilters => {
        ...
        storeSearchPage({
            page: this.griddleName,
            filters: this.state.filters,
            offset: 0,
            size: this.props.dataStore.size
        });

        this.props.dataStore.search(updatedFilters);
    };
