...
import fi.liike.rest.Service.YleissuunnitelmaService;
import fi.liike.rest.api.ContentDto;
import fi.liike.rest.api.SearchContent;
import fi.liike.rest.api.dto.YleissuunnitelmaDto;

@Path("/yleissuunnitelma/")
public class YleissuunnitelmaController extends MainController {

	//private final Logger LOG = LoggerFactory.getLogger(YleissuunnitelmaController.class);
	private YleissuunnitelmaService service;

	public YleissuunnitelmaController() {
		service = new YleissuunnitelmaService();
	}
	...
	public Response create(@Context HttpServletRequest httpRequest, YleissuunnitelmaDto content) throws IOException {
		validate(content);
	...
	public Response create(HttpServletRequest httpRequest, ContentDto content) throws IOException {
		return create(httpRequest, (YleissuunnitelmaDto) content);
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response update(@Context HttpServletRequest httpRequest, YleissuunnitelmaDto content) throws IOException {
		validate(content);
	...
	public Response update(HttpServletRequest httpRequest, ContentDto content) throws IOException {
		return update(httpRequest, (YleissuunnitelmaDto) content);
	}
