...
import fi.liike.rest.Service.RataSuunnitelmaService;
import fi.liike.rest.api.ContentDto;
import fi.liike.rest.api.SearchContent;
import fi.liike.rest.api.dto.RataSuunnitelmaDto;

@Path("/ratasuunnitelma/")
public class RataSuunnitelmaController extends MainController {

	//private final Logger LOG = LoggerFactory.getLogger(RautatienYleissuunnitelmaController.class);
	private RataSuunnitelmaService service;

	public RataSuunnitelmaController() {
		service = new RataSuunnitelmaService();
	}
	...
	public Response create(@Context HttpServletRequest httpRequest, RataSuunnitelmaDto content) throws IOException {
		validate(content);
	...
	public Response create(HttpServletRequest httpRequest, ContentDto content) throws IOException {
		return create(httpRequest, (RataSuunnitelmaDto) content);
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response update(@Context HttpServletRequest httpRequest, RataSuunnitelmaDto content) throws IOException {
		validate(content);
	...
	public Response update(HttpServletRequest httpRequest, ContentDto content) throws IOException {
		return update(httpRequest, (RataSuunnitelmaDto) content);
	}
