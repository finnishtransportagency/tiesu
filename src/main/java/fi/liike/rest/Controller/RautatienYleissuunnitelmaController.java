...
import fi.liike.rest.Service.RautatienYleissuunnitelmaService;
import fi.liike.rest.api.ContentDto;
import fi.liike.rest.api.SearchContent;
import fi.liike.rest.api.dto.RautatienYleissuunnitelmaDto;

@Path("/rautatienyleissuunnitelma/")
public class RautatienYleissuunnitelmaController extends MainController {

	//private final Logger LOG = LoggerFactory.getLogger(RautatienYleissuunnitelmaController.class);
	private RautatienYleissuunnitelmaService service;

	public RautatienYleissuunnitelmaController() {
		service = new RautatienYleissuunnitelmaService();
	}
	...
	public Response create(@Context HttpServletRequest httpRequest, RautatienYleissuunnitelmaDto content) throws IOException {
		validate(content);
	...
	public Response create(HttpServletRequest httpRequest, ContentDto content) throws IOException {
		return create(httpRequest, (RautatienYleissuunnitelmaDto) content);
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response update(@Context HttpServletRequest httpRequest, RautatienYleissuunnitelmaDto content) throws IOException {
		validate(content);
	...
	public Response update(HttpServletRequest httpRequest, ContentDto content) throws IOException {
		return update(httpRequest, (RautatienYleissuunnitelmaDto) content);
	}
