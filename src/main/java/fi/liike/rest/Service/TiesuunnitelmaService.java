...
import fi.liike.rest.Dao.Hibernate.TiesuunnitelmaDaoImpl;
import fi.liike.rest.Dao.MainDao;
import fi.liike.rest.api.*;
import fi.liike.rest.api.dto.TiesuunnitelmaConverter;
...
public class TiesuunnitelmaService extends MainService implements Service {

	private MainDao dao;
	private TiesuunnitelmaConverter converter;
	private final Logger LOG = LoggerFactory.getLogger(TiesuunnitelmaService.class);

	public TiesuunnitelmaService() {
		dao = new TiesuunnitelmaDaoImpl();
		converter = new TiesuunnitelmaConverter();
	}
